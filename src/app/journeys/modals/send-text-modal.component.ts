...
import { ISendEmailData, ITextTemplateEvent } from './../journeys.api.model'
import {
  ISelectOption,
  numberFormControl,
  toSelectOption
...
@IonicPage()
@Component({
  selector: 'send-text-modal',
  templateUrl: 'send-text-modal.component.html'
})
export class SendTextModalComponent implements OnDestroy {
  ...
  constructor(
    ...
    if (viewController.data.action === undefined) {
      ...
      const action: ITextTemplateEvent = viewController.data.action
